...
namespace hlt {
    namespace collision {
		...
		static bool segment_circle_intersect_for_targets(
			const Location& start,
			const Location& end,
			...
			const double fudge)
		{
			...
			// and substitute into the equation of a circle
			// Solve for t
			const double circle_radius = constants::SHIP_RADIUS;
			const double start_x = start.pos_x;
			const double start_y = start.pos_y;
			const double end_x = end.pos_x;
			const double end_y = end.pos_y;
			const double center_x = ship_target.pos_x;
			const double center_y = ship_target.pos_y;
			const double dx = end_x - start_x;
			const double dy = end_y - start_y;
			...
			if (a == 0.0) {
				// Start and end are the same point
				return start.get_distance_to(ship_target) <= circle_radius + fudge;
			}
			...
			const double closest_x = start_x + dx * t;
			const double closest_y = start_y + dy * t;
			const double closest_distance = Location{ closest_x, closest_y }.get_distance_to(ship_target);
