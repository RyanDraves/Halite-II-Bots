...
/*
	Folder Name:	Last Version + Adjusts + 4-player noob rush implementation (+Rank 22)
	Last Versions:	Last Version + Distraction Prevention (Rank 21)
					Rank 22 : Outnumbered Defense + Bunny Ship (v2.1) + Collision Rounding Fixes
 */
...
/*
    Terms referenced:
		...
		-Strafing: Reference to, in all seriousness, the Minecraft term to mean "attack while moving sideways."
 */

/*
	After a while I gave up typing out hlt::constants:: for every number, or several if I needed to add them, so here's a reference of some used.
	7.0: max speed		19.0-19.x: 2*7.0 + 5.0	0.5: ship radius	1.1: 0.5 + 0.6
	5.0: attack radius	12.0-12.x: 7.0 + 5.0	0.6: 0.5 + fudge
*/

/*
	Major issues:

	Note: Allied bunny ships need to be better at running away (remove 7 in from edge).

	Note: Allied bunny ships are running into enemy docked ships.
	...
	Idea: Retest collision loop refactoring.
	Note: Enemy "bunny ships" are distracting and build up a large, unproductive swarm that can't catch it when I'm defending.
	Note: Noob rushes for 4v4's.
 */

/*
	Minor issues:

	Note: Fringe case that an unfull planet can send ships away with distraction protection, but a full planet the next turn sends them back, making a loop.

	Idea: If the "hunter" for allied bunny ships is isolated, chase him when he's close instead of running away.
...
int main() {
    ...
    for (;;) {
        ...
        //if (map.ships.size() == 2) {
            ...
            if (first_ship.docking_status == hlt::ShipDockingStatus::Undocked && (NoobRush(first_ship, first_enemy, first_planet, map.ships.size()) || special_id != 999)) {
...
bool NoobRush(const hlt::Ship& ship, const hlt::Ship& enemy, const hlt::Planet& planet, const unsigned int players) {
	...
	if (players == 2) {
		...
		if ((int)(enemy.location.get_distance_to(docking_spot) / 7) - (int)(ship.location.get_distance_to(docking_spot) / 7) <= 10) {
			return true;
		}
	}
    return false;
...
hlt::Location Strafe(const hlt::Ship& ship, const hlt::Ship& target, const hlt::Ship& hunter, const hlt::Map& map) {
	...
	double distance_to_target = ship.location.get_distance_to(ship.location.get_closest_point(target.location, target.radius - 2));
	double distance;
	if (distance_to_target >= 7.0) {
		distance = 7.0;
	}
	else {
		double remainder = distance_to_target - (int)distance_to_target;
		//Might want to raise this number as the ship does not check for collisions with enemy ships.
		if (remainder >= 0.5) {
			distance = (int)distance_to_target + 1;
		}
		else {
			distance = (int)distance_to_target;
		}
	}
	double left_dx = cos(angle_deg * (M_PI / 180) + (M_PI / 2.0)) * distance;
	double left_dy = sin(angle_deg * (M_PI / 180) + (M_PI / 2.0)) * distance;
	double right_dx = cos(angle_deg * (M_PI / 180) - (M_PI / 2.0)) * distance;
	double right_dy = sin(angle_deg * (M_PI / 180) - (M_PI / 2.0)) * distance;
...
bool Outnumbered(const hlt::Ship& ship, const hlt::Map& map, hlt::PlayerId player_id) {
	...
	for (const auto& player_ship : map.ships) {
		for (const hlt::Ship& s : player_ship.second) {
			if (s.owner_id == player_id && ship.location.get_distance_to(s.location) <= 5.0 && s.docking_status == hlt::ShipDockingStatus::Undocked) {
				allies++;
			}
			else if (s.owner_id != player_id && ship.location.get_distance_to(s.location) <= 12 && s.docking_status == hlt::ShipDockingStatus::Undocked) {
				enemies++;
			}
		}
	}
