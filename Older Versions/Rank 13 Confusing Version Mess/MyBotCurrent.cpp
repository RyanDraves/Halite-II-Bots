...
/*
	...
	Note: Defending allies that are Outnumbered find the nearest ally, not the docked ship closest to the attacker. Sometimes the attacker can still pick off a docked ship.
	Idea: Implement this and compare to see if it's better.
	...
	Note: Not very good at detecting noob rushes. Sometimes an allied bunny ship is too far away to be effective.
	Idea: This might be resolved by improving the bunny ship by the idea above, and/or refactoring NoobRush.
 */
...
/*
	...
	Idea: If the ship just can't find a place 12 away from other ships, try again without HunterDanger just to make sure it moves, even if it takes a hit.

	Idea: To improve strategic attacks, go for the docked ships over nearest enemies when I outproduce that enemy bot.
...
bool NoobManeuver(const bool noob_rush, const std::vector<hlt::Location> targets, const unsigned int index);
...
int main() {
    ...
    for (;;) {
		...
		if (move_number < 3 || noob_rush) {
			...
			if (map.ships.at(player_id).size() > map.ships.at(nearest_enemy.owner_id).size()) {
				...
				if ((units_closer > 0 && enemy_distance < 98) || enemy_distance < 70) {
					...
					bool left = true;
					while (TargetOrPlanetInTheWay(unit_leader.location, ship_target, map, ships, targets, distances, 0, true) && max_corrections) {
						max_corrections--;
					...
					/*if (dont_move) {
						//Get around the planet a bit better.
						hlt::Location ship_target = Strafe(unit_leader, nearest_enemy, nearest_enemy, map);
						targets.push_back(SetTarget(unit_leader, ship_target));
						ships.push_back(unit_leader);
						distances.push_back(unit_leader.location.get_distance_to(ship_target));
					}
		...
		if (first_ship.docking_status == hlt::ShipDockingStatus::Undocked && (NoobRush(first_ship, first_enemy, first_planet, map.ships.size()) || bunny_id != 999) && !defeated) {
			bunny_id = first_ship.entity_id;
			...
			if (hunter.entity_id == first_ship.entity_id) {
				...
				if (hunter.location.get_distance_to(first_ship.location) >= 19.1) {
					...
					//If there are no docked ships for any player,
		...
		for (const auto& player_ship : map.ships) {
			...
			for (const hlt::Ship& s : player_ship.second) {
				if (s.owner_id != player_id && !noob_rush && !defeated) {
					//Find the distance of the closest ally planet, returns 999 if the closest planet is not an ally planet.
					/*double dist = DistanceToAllyPlanet(s, map.planets, player_id);
					if (dist < 50) {
        ...
        for (const hlt::Ship& ship : map.ships.at(player_id)) {
			...
			*/
			...
			if (nearest_planet.owner_id == player_id) {
				bool end_loop = false;
				for (unsigned int i = 0; i < attacking_enemies.size(); i++) {
					if (ClosestAllyToEnemy(ship, map.ships.at(player_id), attacking_enemies[i], defending_allies) && !end_loop) {
						end_loop = true;
						if (Outnumbered(ship, map, player_id)) {
							...
							hlt::Location ship_target = ship.location.get_closest_point(attacking_enemies[i].location, attacking_enemies[i].radius - 2);
							targets.push_back(SetTarget(ship, ship_target));
							ships.push_back(ship);
							distances.push_back(ship.location.get_distance_to(ship_target));
							
						}
						attacking_enemies.erase(attacking_enemies.begin() + i);
						defending_allies.push_back(ship.entity_id);
					}
				}
				if (end_loop) {
					continue;
		...
		do {
			...
			for (unsigned int i = 0; i < targets.size(); i++) {
				...
				if (targets[i] == ships[i].location || dont_move) {
					continue;
				}
				...
				//Regular ship collision check.
					...
					/*bool is_unit_leader = false;
					if (ships[i].entity_id == unit_leader_id) {
						is_unit_leader = true;
					}*/
					while ((TargetOrPlanetInTheWay(ships[i].location, targets[i], map, ships, targets, distances, i, false) || OutOfBounds(targets[i], map) || NoobManeuver(noob_rush, targets, i)) && max_corrections) {
						correction = true;
						max_corrections--;
						const double distance = ships[i].location.get_distance_to(targets[i]);
						const double angle_rad = ships[i].location.orient_towards_in_rad(targets[i]);
				...
				if (!max_corrections) {
					//Reset the unit leader for a noob rush.
					if (noob_rush) {
						unit_leader_id = 999;
						dont_move = true;
					}
		...
		for (unsigned int i = 0; i < targets.size(); i++) {
			...
			if (ships[i].location == targets[i] || dont_move) {
				continue;
			}
...
bool NoobManeuver(const bool noob_rush, const std::vector<hlt::Location> targets, const unsigned int index) {
	if (noob_rush) {
		double closest = NO_RADIUS;
		for (hlt::Location t : targets) {
			if (t.pos_x != targets[index].pos_x || t.pos_y != targets[index].pos_y) {
				double dist = t.get_distance_to(targets[index]);
				if (dist < closest) {
					closest = dist;
				}
			}
...
bool TargetOrPlanetInTheWay(const hlt::Location& location, const hlt::Location& target, const hlt::Map& map, std::vector<hlt::Ship> ships, std::vector<hlt::Location> targets, std::vector<double> distances, unsigned int index, bool unit_leader) {
	if (unit_leader) {
		for (const hlt::Planet& planet : map.planets) {
			const hlt::Location planet_edge = target.get_closest_point(planet.location, planet.radius);
			if (hlt::navigation::check_planet_between(location, target, planet, true) || target.get_distance_to(planet_edge) < 3.5) {
				return true;
			}
		}
	}
