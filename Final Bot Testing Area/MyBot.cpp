...
/*
	...
	Note: This failed.
	Idea: Try to some crazy idea at the last minute of submission.
 */
...
int main() {
    ...
    for (;;) {
		...
		if (move_number < 3 || noob_rush) {
			...
			if (map.ships.at(player_id).size() > map.ships.at(nearest_enemy.owner_id).size()) {
				...
				if ((units_closer > 0 && enemy_distance < 98) || enemy_distance < 70) {
					...
					hlt::Location ship_target = unit_leader.location.get_closest_point(nearest_enemy.location, 0);
		...
		//
		//No comments, just written all at once. Not my problem!
		...
		for (unsigned int i = 0; i < each_ship.size(); i++) {
			if (!IsInShipVector(each_ship[i], checked)) {
				...
				for (unsigned int a = 0; a < ally_distances.size(); a++) {
					in << ally_distances[a] << " ";
				}
				hlt::Log::log(in.str());
		...
		for (unsigned int i = 0; i < outnumbered.size(); i++) {
			information << outnumbered[i] << " ";
		}
		hlt::Log::log(information.str());
